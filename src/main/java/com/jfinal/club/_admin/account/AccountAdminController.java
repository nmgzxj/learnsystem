...
public class AccountAdminController extends BaseController {
	...
	public void save() {
		...
		account.setIp(IpKit.getRealIp(getRequest()));
	...
	public void resetPass() {
		Ret ret = srv.resetPass(srv.findById(getParaToInt("id")));
		renderJson(ret);
	}
	
	public void modifyPassword() {
		setAttr("account", srv.findById(getParaToInt("id")));
		render("modify_password.html");
	}
	/**
	 * 保存修改的密码
	 */
	@Before(AccountModifyPasswordValidator.class)
	public void savePasswd() {
		Ret ret = srv.modifyPass(srv.findById(getParaToInt("id")), getPara("new_pass"));
		renderJson(ret);
	}
}
